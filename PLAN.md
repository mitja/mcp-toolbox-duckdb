...
- **✅ DECIMAL scale preservation.** `Source.executeQuery` now captures the declared scale from each `DECIMAL(p,s)` column. duckdb-go-bindings does not implement `sql.ColumnType.DecimalSize()`, so a regex parses the scale out of the `DatabaseTypeName()` string ("DECIMAL(38,2)" → 2) as a fallback. `renderValue` then pads `duckdb.Decimal.String()`'s trimmed output to the declared scale (`1370.50` stays `"1370.50"`, `1000.00` stays `"1000.00"`). Unit tests pin both the regex (10 cases) and the formatter (12 cases including integer, zero, negative, already-at-scale, and shorter-than-scale inputs). The existing `TestDecimalRenderedAsString` integration test is updated to assert the new behavior.
...
- ~~Metadata tools~~ — landed in Phase 2 (see top). `duckdb-quack-whoami` deferred.
