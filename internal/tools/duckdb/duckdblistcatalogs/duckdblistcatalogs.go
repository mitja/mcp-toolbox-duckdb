...
// Package duckdblistcatalogs implements "duckdb-list-catalogs", a
// parameterless metadata tool that returns the catalogs attached to the
// source's client-side DuckDB. For a typical duckdb-quack source this is
// the alias the source ATTACHed (`remote` by default) plus the built-in
// `system` and `temp` catalogs.
package duckdblistcatalogs
...
const resourceType string = "duckdb-list-catalogs"

// statement is the client-side query: it asks the in-process DuckDB which
// catalogs are visible to it, which for a duckdb-quack source is the
// ATTACHed alias plus the built-in `system` / `temp` catalogs.
const statement = `SELECT catalog_name FROM information_schema.schemata GROUP BY catalog_name ORDER BY catalog_name`
...
// Config is the YAML-decoded configuration for one duckdb-list-catalogs
// tool. There is no agent-facing parameter: the tool always returns the
// full list of catalogs visible to its source.
type Config struct {
...
func (t Tool) Invoke(ctx context.Context, mgr tools.SourceProvider, _ parameters.ParamValues, _ tools.AccessToken) (any, util.ToolboxError) {
	...
	resp, runErr := duckdbmeta.Invoke(ctx, src, t.Source, t.statementHash, func(ctx context.Context, opts duckdbquack.QueryOptions) (*duckdbquack.QueryResult, error) {
		return src.RunSQL(ctx, statement, nil, opts)
	})
	if runErr != nil {
